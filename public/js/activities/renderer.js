  ...
  _renderClassifyTwo(q, container) {
    ...
    container.querySelectorAll('.order-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        const zone = container.querySelector(`.classify-zone[data-cat="${cat}"] .zone-items`);
        if (zone) {
          const mini = document.createElement('span');
          mini.style.cssText = 'font-size:1.5rem;background:rgba(255,255,255,0.2);border-radius:8px;padding:4px 8px;';
          mini.textContent = item.textContent.trim().split(' ')[0];
          zone.appendChild(mini);
          placed[item.dataset.name] = cat;
          item.style.opacity = '0.3';
          item.style.pointerEvents = 'none';
          check();
        }
