...
const generators = {
  ...
  'object-color': () => {
    ...
    return shuffle(items).slice(0, 5).map(item => ({
      type: 'object-color',
      question: `¿De qué color es la ${item.obj}?`,
      emoji: item.emoji,
      options: shuffle([item.color, ...["Rojo","Azul","Verde","Amarillo","Naranja","Morado"].filter(c => c !== item.color).slice(0,3)]),
      correct: item.color
    }));
  },

  'memory-color': () => {
    const pairs = sample(COLORS_DATA, 4).map(c => ({ name: c.name, emoji: c.emoji, hex: c.hex }));
    return [{ type: 'memory', pairs, gridCols: 4 }];
  },

  'classify-color': () => {
    const warm = COLORS_DATA.filter(c => ['Rojo','Naranja','Amarillo','Rosa'].includes(c.name));
    const cool = COLORS_DATA.filter(c => ['Azul','Verde','Morado'].includes(c.name));
    return [{ type: 'classify-two', question: '¿Cálido o frío?', categories: ['🔥 Cálido', '❄️ Frío'], items: [...sample(warm,3).map(c=>({...c,cat:'🔥 Cálido'})),...sample(cool,3).map(c=>({...c,cat:'❄️ Frío'}))] }];
  },
  ...
  'recognize-vocal': () => {
    return shuffle(VOWELS_DATA).map(v => ({
      ...
      display: v.letter,
      options: shuffle([v.letter, ...VOWELS_DATA.filter(x=>x.letter!==v.letter).map(x=>x.letter).slice(0,3)]),
      correct: v.letter
    }));
  },
  ...
  'listen-vocal': () => {
    return shuffle(VOWELS_DATA).map(v => ({
      ...
      vocal: v.letter,
      options: shuffle([v.letter, ...VOWELS_DATA.filter(x=>x.letter!==v.letter).map(x=>x.letter)]),
      correct: v.letter
    }));
  },
  ...
  'memory-vocal': () => {
    const pairs = VOWELS_DATA.map(v => ({ name: v.letter, emoji: v.words[0].emoji }));
    return [{ type: 'memory', pairs, gridCols: 4 }];
  },
  ...
  'recognize-animal': () => {
    ...
    return sample(allAnimals, 6).map(a => ({
      ...
      emoji: a.emoji,
      options: shuffle([a.name, ...pickWrong(allAnimals, a, 3).map(x=>x.name)]),
      correct: a.name
    }));
  },
  ...
  'sound-animal': () => {
    ...
    return sample(withSounds, 5).map(a => ({
      ...
      sound: a.sound,
      options: shuffle([a.name, ...pickWrong(withSounds, a, 3).map(x=>x.name)]),
      correct: a.name
    }));
  },
  ...
  'classify-animal': () => {
    const items = [
      ...
      ...ANIMALS_DATA.wild.slice(0,3).map(a=>({...a,cat:'🌿 Salvaje'}))
    ];
    return [{ type: 'classify-two', question: '¿Doméstico o salvaje?', categories: ['🏠 Doméstico','🌿 Salvaje'], items: shuffle(items) }];
  },

  'memory-animal': () => {
    const pairs = sample([...ANIMALS_DATA.domestic,...ANIMALS_DATA.wild], 4).map(a=>({ name: a.name, emoji: a.emoji }));
    return [{ type: 'memory', pairs, gridCols: 4 }];
  },

  'habitat-animal': () => {
    const animals = sample([...ANIMALS_DATA.domestic,...ANIMALS_DATA.wild,...ANIMALS_DATA.birds], 5);
  ...
  'count-objects': () => {
    return Array.from({length:6}, (_, i) => {
      ...
      return {
        ...
        emoji, count,
        options: shuffle([count, ...new Set([count-1,count+1,count+2,count-2].filter(n=>n>0&&n<=20))].slice(0,4)),
        correct: count
      };
    });
  },
  ...
  'select-number': () => {
    return Array.from({length:6}, () => {
      ...
      return {
        ...
        options: shuffle([n,...new Set([n-2,n-1,n+1,n+2].filter(x=>x>0&&x<=20))].slice(0,4)),
        correct: n
      };
    });
  },
  ...
  'number-sequence': () => {
    return Array.from({length:5}, () => {
      ...
      return {
        ...
        sequence: seq,
        options: shuffle([start+3, start+5, start+1, start+6]).slice(0,4),
        correct: start+3
      };
    });
  },
  ...
  'recognize-shape': () => {
    return sample(SHAPES_DATA, 5).map(s => ({
      ...
      emoji: s.emoji,
      options: shuffle([s.name, ...pickWrong(SHAPES_DATA, s, 3).map(x=>x.name)]),
      correct: s.name
    }));
  },

  'classify-shape': () => {
    const items = SHAPES_DATA.map(s => ({ ...s, cat: s.sides === 0 ? '⭕ Sin esquinas' : s.sides === 3 ? '3️⃣ 3 lados' : '4️⃣ 4 lados' }));
    return [{ type: 'classify-three', question: 'Clasifica la figura', categories: ['⭕ Sin esquinas','3️⃣ 3 lados','4️⃣ 4 lados'], items: shuffle(items) }];
  },
  ...
  'sum-objects': () => {
    return Array.from({length:6}, () => {
      ...
      const emoji = emojis[Math.floor(Math.random()*emojis.length)];
      return {
        type: 'sum-objects', a, b, total, emoji,
        question: `¿Cuántos hay en total?`,
        options: shuffle([total,...[total-1,total+1,total+2].filter(n=>n>0)].slice(0,4)),
        correct: total
      };
    });
  },
  ...
  'sum-numbers': () => {
    return Array.from({length:6}, () => {
      ...
      const total = a+b;
      return {
        type: 'sum-numbers', a, b, total,
        question: `¿Cuánto es ${a} + ${b}?`,
        options: shuffle([total,...[total-1,total+1,total+2,total-2].filter(n=>n>=0)].slice(0,4)),
        correct: total
      };
    });
  },
  ...
  'sub-numbers': () => {
    return Array.from({length:6}, () => {
      ...
      const res = a-b;
      return {
        type: 'sub-numbers', a, b, res,
        question: `¿Cuánto es ${a} - ${b}?`,
        options: shuffle([res,...[res-1,res+1,res+2,res-2].filter(n=>n>=0)].slice(0,4)),
        correct: res
      };
    });
  },
  ...
  'eng-colors': () => {
    return sample(COLORS_DATA, 6).map(c => ({
      ...
      emoji: c.emoji,
      hex: c.hex,
      options: shuffle([c.eng, ...pickWrong(COLORS_DATA, c, 3).map(x=>x.eng)]),
      correct: c.eng
    }));
  },
  ...
  'eng-greetings': () => {
    return shuffle(GREETINGS_DATA).map(g => ({
      ...
      emoji: g.emoji,
      options: shuffle([g.esp,...GREETINGS_DATA.filter(x=>x.eng!==g.eng).map(x=>x.esp).slice(0,3)]),
      correct: g.esp
    }));
  },
  ...
  'eng-body': () => {
    return sample(BODY_PARTS, 6).map(b => ({
      ...
      emoji: b.emoji,
      options: shuffle([b.eng,...pickWrong(BODY_PARTS,b,3).map(x=>x.eng)]),
      correct: b.eng
    }));
  },

  'eng-memory': () => {
    const pairs = sample(COLORS_DATA, 4).map(c => ({ name: c.esp || c.name, emoji: c.emoji, pair: c.eng }));
  ...
  'memory-4': () => {
    ...
    return [{ type: 'memory', pairs: items.map(x=>({name:x.name,emoji:x.emoji})), gridCols: 4 }];
  },

  'memory-6': () => {
    const items = sample([...ANIMALS_DATA.domestic,...ANIMALS_DATA.wild,...FRUITS_DATA], 6);
    return [{ type: 'memory', pairs: items.map(x=>({name:x.name,emoji:x.emoji})), gridCols: 4 }];
  },

  'memory-8': () => {
    const items = sample([...ANIMALS_DATA.domestic,...ANIMALS_DATA.wild,...FRUITS_DATA,...SHAPES_DATA], 8);
    return [{ type: 'memory', pairs: items.map(x=>({name:x.name,emoji:x.emoji})), gridCols: 4 }];
  },
  ...
  'color-pattern': () => {
    ...
    return Array.from({length:4}, () => {
      ...
      return {
        ...
        options: shuffle([correct, ...pat.filter(x=>x!==correct), '🟣','🟤'].slice(0,4)),
        correct
      };
    });
  },
  ...
  'logic-odd': () => {
    ...
    return shuffle(groups).slice(0,4).map(g => ({
      type: 'odd-one-out',
  ...
  'identify-body': () => {
    return sample(BODY_PARTS, 5).map(b => ({
      ...
      emoji: b.emoji,
      options: shuffle([b.name,...pickWrong(BODY_PARTS,b,3).map(x=>x.name)]),
      correct: b.name
    }));
  },
  ...
  'identify-transport': () => {
    return sample(TRANSPORT_DATA, 5).map(t => ({
      ...
      emoji: t.emoji,
      options: shuffle([t.name,...pickWrong(TRANSPORT_DATA,t,3).map(x=>x.name)]),
      correct: t.name
    }));
  },
  ...
  'word-image': () => {
    ...
    return sample(all, 6).map(item => ({
      ...
      word: item.name,
      emoji: item.emoji,
      options: shuffle([item, ...sample(all.filter(x=>x.name!==item.name),3)]).map(x=>x.emoji),
      correct: item.emoji
    }));
  },
  ...
  // Fallback
  'default': () => {
    return [{ type: 'default', question: 'Actividad en desarrollo 🚧', options: [], correct: null }];
  }
};

// Main generate function
export function generateActivity(activityId, activityType) {
  const generator = generators[activityType] || generators[activityId] || generators['default'];
  try {
